...
public interface VideoStorage {
    ...
    /**
     * 트랜스코딩 결과물을 저장소에 업로드 (디렉토리 단위)
     * @param localDir          업로드할 로컬 디렉토리 (HLS 파일들이 들어있음)
     * @param destinationPrefix 저장소 내 목적지 경로 (예: "contents/1/transcoded/360p")
    ...
    /**
     * 단일 파일을 저장소에 업로드
     * @param file           업로드할 로컬 파일 (예: master.m3u8)
     * @param destinationKey 저장소 내 목적지 키 (예: "contents/1/transcoded/master.m3u8")
     */
    void putFile(Path file, String destinationKey);
}
