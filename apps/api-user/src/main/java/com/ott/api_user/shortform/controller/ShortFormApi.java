...
public interface ShortFormApi {
    // 1. 숏폼 피드 목록 조회 API
    @Operation(summary = "숏폼 피드 목록 조회", description = "무한 스크롤을 위한 숏폼 피드를 반환합니다. (추천 80% + 인기 20% 하이브리드)")
    @ApiResponses(value = {
    ...
    ResponseEntity<SuccessResponse<PageResponse<ShortFormFeedResponse>>> getShortFormFeed(
            ...
            @PositiveOrZero @Parameter(description = "페이지 번호 (0부터 시작)", example = "0") @RequestParam(defaultValue = "0") int page,
            @Positive @Parameter(description = "패이지 크기", example = "10") @RequestParam(defaultValue = "10") int size
    );
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "시청 이벤트 기록 성공", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = SuccessResponse.class)) }),
            @ApiResponse(responseCode = "404", description = "존재하지 않는 숏폼 ID", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class)) })
    })
    @PostMapping("/events")
    ResponseEntity<SuccessResponse<Void>> recordShortFormView(
            @Parameter(hidden = true) @AuthenticationPrincipal Long memberId,
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "CTA 클릭 이벤트 기록 성공", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = SuccessResponse.class)) }),
            @ApiResponse(responseCode = "404", description = "존재하지 않는 숏폼 ID", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class)) })
    })
